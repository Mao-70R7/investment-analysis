...
$jobRoot = Join-Path $ProjectRoot ("data\\raw\\ttfund\\incremental_history_runs\\{0}\\{1}" -f $day, $runId)
...
try {
    ...
    Invoke-Stage -StageName "01_history_drive" -CommandArgs @(
    ...
    Invoke-Stage -StageName "02_collect_after_history" -CommandArgs @(
    ...
    Write-Status -Stage "done" -State "completed" -Message "incremental history repair finished"
}
catch {
