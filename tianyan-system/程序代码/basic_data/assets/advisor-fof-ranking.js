(() => {
  ...
  function benchmarkBucket(row) {
    return row?.broadEquityBucket || row?.riskProfile?.broadEquityBucket || row?.benchmarkEquityBucket || row?.riskProfile?.benchmarkEquityBucket || row?.rankingCategory || "未分档";
  }
  ...
  function filterRows() {
    return rows.filter((row) => {
      ...
      if (state.category !== "all" && benchmarkBucket(row) !== state.category) return false;
  ...
  function renderScatterDetail(row, xMetric, yMetric) {
    ...
    const profileItems = [
      { key: "bucket:benchmarkEquity", label: "基准风险资产权重", value: benchmarkBucket(row), format: "text" },
      { key: "risk:benchmarkRiskAsset", label: "基准风险资产权重比例", value: riskProfileValue(row, "broadEquityWeight") ?? riskProfileValue(row, "benchmarkEquityWeight") },
    ...
    return `<div class="advisor-scatter-detail">
      ...
      <div class="advisor-scatter-detail-grid">
        ...
        ${profileItems.map((item) => `<div><span>${B.esc(item.label)}</span><b>${fmtMetric(item.value, item)}</b></div>`).join("")}
        <div><span>基准风险资产权重</span><b>${B.esc(benchmarkBucket(row))}</b></div>
  ...
  function tableRows(displayRows) {
    ...
    return displayRows.map((row) => {
      ...
      return `<tr class="${row.isGuangfa ? "is-gf-row" : ""}">
        ...
        <td>${B.esc(benchmarkBucket(row))}<div class="small">${B.esc(row.broadEquityNote || row.rankingCategoryBasis || "权益+商品+另类风险资产合计口径")}</div></td>
  ...
  function renderControls() {
    ...
    const categories = Array.from(new Set(rows.map((row) => benchmarkBucket(row)).filter(Boolean))).sort((a, b) => {
      const left = /^L\d+$/.test(a) ? Number(a.slice(1)) : 999;
      const right = /^L\d+$/.test(b) ? Number(b.slice(1)) : 999;
      return left - right || String(a).localeCompare(String(b), "zh-CN");
    });
    return `<section class="panel advisor-rank-controls">
      ...
      <div class="advisor-rank-filter-grid">
        ...
        <label class="strategy-filter-field"><span>基准风险资产权重</span><select id="rankCategory">${optionHtml(categories, state.category, "全部分档")}</select><em>按权益、商品和另类风险资产合计权重分档并混排</em></label>
  ...
  function renderCategorySummary() {
    ...
    rows.forEach((row) => {
      const bucket = benchmarkBucket(row);
      if (!grouped.has(bucket)) grouped.set(bucket, []);
      grouped.get(bucket).push(row);
    });
    const categories = [...grouped.entries()]
      .map(([bucket, list]) => ({ 分类: bucket, 产品数: list.length, 投顾策略数: list.filter((row) => row.entityType === "投顾策略").length, FOF基金数: list.filter((row) => row.entityType === "FOF基金").length }))
      .sort((a, b) => {
        const left = /^L\d+$/.test(a.分类) ? Number(a.分类.slice(1)) : 999;
        const right = /^L\d+$/.test(b.分类) ? Number(b.分类.slice(1)) : 999;
        return left - right || a.分类.localeCompare(b.分类, "zh-CN");
      })
      .slice(0, 12);
    return `<section class="panel">
      <div class="panel-head"><div><h2>主要分档样本</h2><p class="desc">用于判断当前可比池是否足够大；排名时可在“基准风险资产权重”中进一步收窄。</p></div></div>
  ...
  function render() {
    ...
    root.innerHTML = `
      <section class="page-title">
        <div>
          <h1>投顾-FOF排名</h1>
          <p class="desc">投顾策略与FOF基金统一进入基准风险资产权重池，可按收益区间、产品类型和分档混合排名。</p>
        </div>
