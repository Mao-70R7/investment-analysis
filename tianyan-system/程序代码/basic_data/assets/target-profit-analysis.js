(() => {
  ...
  const starPoints = (cx, cy, outerRadius, innerRadius = outerRadius * 0.45) => Array.from({ length: 10 }, (_, index) => {
  ...
  function renderScatter(seriesRows, periodRows, statusBasePeriods, allSeriesRows = pack.series || [], allPeriodRows = pack.periods || []) {
    ...
    const pointState = (row) => {
      ...
      const drawRank = selected ? 6 : advisorHighlighted ? 5 : gfAdvisor ? 4 : filterMatched ? 2 : muted ? 0 : 1;
      return { selected, advisorHighlighted, gfAdvisor, filterMatched, muted, drawRank };
    };
    ...
    return `
      <section class="panel target-scatter-panel">
        ...
        <div class="topic-scatter-wrap target-scatter-wrap">
          ...
          <div class="target-scatter-hint"><span><i class="is-active"></i>当前筛选命中/高亮</span><span><i></i>未命中筛选的背景系列</span><span><i class="is-gf"></i>广发基金（五角星）</span></div>
          <svg class="target-dot-plot target-series-dot-plot" viewBox="0 0 ${width} ${height}" role="img" aria-label="目标盈系列点阵">
            ...
            ${pointRows.map((row) => {
              ...
              const gfAdvisor = point.gfAdvisor;
              ...
              if (gfAdvisor) {
                return `<polygon class="topic-scatter-point target-dot target-gf-star ${cls} is-gf-advisor" points="${starPoints(x, y, dotRadius + 2.4)}" fill="${fill}" data-select-series="${esc(row.系列ID)}" data-tip="${esc(tip)}"><title>${esc(tip)}</title></polygon>`;
              }
              return `<circle class="topic-scatter-point target-dot ${cls}" cx="${x.toFixed(2)}" cy="${y.toFixed(2)}" r="${dotRadius.toFixed(2)}" fill="${fill}" data-select-series="${esc(row.系列ID)}" data-tip="${esc(tip)}"><title>${esc(tip)}</title></circle>`;
            }).join("")}
          </svg>
  ...
  function renderLifecycleChart(periodRows, statusBasePeriods) {
    ...
    const pointRows = [...rows].sort((a, b) => Number(isGfAdvisor(a)) - Number(isGfAdvisor(b)));
        ...
        <div class="topic-scatter-wrap target-scatter-wrap">
          ...
          <svg class="target-dot-plot target-detail-dot-plot" viewBox="0 0 ${width} ${height}" role="img" aria-label="目标盈明细点阵">
            ...
            ${pointRows.map((row) => {
              ...
              const gfAdvisor = isGfAdvisor(row);
              const fill = muted ? "#cbd5e1" : style.color;
              if (gfAdvisor) {
                return `<polygon class="topic-scatter-point ${cls} is-gf-advisor target-dot target-gf-star target-life-dot target-life-dot-${style.key}" points="${starPoints(x, y, radius + 2.2)}" fill="${fill}" data-select-period="${esc(row.统一策略ID)}" data-select-series="${esc(row.系列ID)}" data-tip="${esc(tip)}"><title>${esc(tip)}</title></polygon>`;
              }
              return `<circle class="topic-scatter-point ${cls} target-dot target-life-dot target-life-dot-${style.key}" cx="${x.toFixed(2)}" cy="${y.toFixed(2)}" r="${radius}" fill="${fill}" data-select-period="${esc(row.统一策略ID)}" data-select-series="${esc(row.系列ID)}" data-tip="${esc(tip)}"><title>${esc(tip)}</title></circle>`;
            }).join("")}
          </svg>
  ...
  root.addEventListener("click", (event) => {
    const target = event.target.closest("button, circle, polygon");
    if (!target) return;
