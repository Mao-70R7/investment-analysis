(() => {
  ...
  const qualityPack = window.__BASIC_DATA_QUALITY_PACK__ || {};
  const qualityChecks = Array.isArray(qualityPack.checks) ? qualityPack.checks : [];
  const qualityWarnings = qualityChecks.filter((row) => {
    const status = raw(row.状态 || row.status);
    return ["warn", "error", "bad"].includes(status);
  });
  const dataUpdatedText = overview.数据更新至 || overview.最新数据日期 || "未披露";
  const generatedText = overview.数据刷新时间 || overview.生成时间 || qualityPack.generatedAt || "未披露";
  const channelCount = Number(overview.接入渠道数 || 0);
  const ttfundStrategyCount = Number(overview.天天策略数 || 0);
  const officialPerformanceCount = Number(overview.有官方业绩策略数 || 0);
  const historyRebalanceCount = Number(overview.有历史调仓策略数 || 0);
  const replayStrategyCount = Number(overview.纳入回放策略数 || 0);
  const relatedFundCount = Number(overview.基金数 || 0);
  const fundNavRows = Number(overview.基金净值行数 || 0);
  const fundDividendRows = Number(overview.基金分红事件数 || 0);
  const missingStrategyFundNav = Number(overview.策略基金净值缺失数 || 0);

  function statTile(title, value, desc, tone = "") {
    return `<article class="system-stat-tile${tone ? ` tone-${B.esc(tone)}` : ""}">
      <span>${B.esc(title)}</span>
      <strong>${B.esc(value)}</strong>
      <small>${B.esc(desc)}</small>
    </article>`;
  }

  function tagList(items) {
    return `<div class="system-tag-list">${items.map((item) => `<span>${B.esc(item)}</span>`).join("")}</div>`;
  }

  function dataLane(title, volume, rawItems, derivedItems, tone) {
    return `<article class="system-data-lane tone-${B.esc(tone)}">
      <header><div><span>${B.esc(title)}</span><strong>${B.esc(volume)}</strong></div><em>${tone === "strategy" ? "投顾策略" : "公募基金"}</em></header>
      <div class="system-lane-body">
        <section class="system-data-stage">
          <b>已采集的事实数据</b>
          ${tagList(rawItems)}
        </section>
        <i class="system-lane-arrow" aria-hidden="true">→</i>
        <section class="system-data-stage is-derived">
          <b>加工得到的统一数据</b>
          ${tagList(derivedItems)}
        </section>
      </div>
    </article>`;
  }

  function coverageBar(title, value, total, desc) {
    const rate = total > 0 ? Math.max(0, Math.min(100, value / total * 100)) : 0;
    return `<div class="system-coverage-row">
      <div><strong>${B.esc(title)}</strong><span>${B.esc(desc)}</span></div>
      <div class="system-coverage-value"><b>${count(value)} / ${count(total)}</b><em>${rate.toFixed(1)}%</em></div>
      <div class="system-coverage-meter" role="progressbar" aria-label="${B.esc(title)}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${rate.toFixed(1)}"><i style="width:${rate.toFixed(1)}%"></i></div>
    </div>`;
  }

  function architectureNode(index, title, body, tone = "") {
    return `<article class="system-architecture-node${tone ? ` tone-${B.esc(tone)}` : ""}">
      <span>${B.esc(index)}</span><strong>${B.esc(title)}</strong><small>${B.esc(body)}</small>
    </article>`;
  }

  function hierarchyStep(level, title, body, tone) {
    return `<article class="system-hierarchy-step tone-${B.esc(tone)}">
      <span>${B.esc(level)}</span><div><strong>${B.esc(title)}</strong><small>${B.esc(body)}</small></div>
    </article>`;
  }

  function functionCard(title, body, href, action = "进入") {
    return `<article class="system-function-card">
      <div><strong>${B.esc(title)}</strong><span>${B.esc(body)}</span></div>
      <a class="link" href="${B.esc(href)}">${B.esc(action)}</a>
    </article>`;
  }

  function visualSection(title, desc, body, className = "") {
    return `<section class="system-visual-section${className ? ` ${B.esc(className)}` : ""}">
      <header class="system-section-head"><div><h2>${B.esc(title)}</h2><p>${B.esc(desc)}</p></div></header>
      ${body}
    </section>`;
  }

  function pageCard(title, body, href, action = "进入") {
    return `<article class="system-page-card">
      <div><strong>${B.esc(title)}</strong><p>${B.esc(body)}</p></div>
      <a class="link" href="${B.esc(href)}">${B.esc(action)}</a>
    </article>`;
  }

  function discussionItem(title, body) {
    return `<div class="system-discussion-item"><strong>${B.esc(title)}</strong><span>${B.esc(body)}</span></div>`;
  }

  function systemModule(title, desc, body, open = false) {
    return `<details class="panel fold-block system-module"${open ? " open" : ""}>
      <summary><span>${B.esc(title)}</span><em>${B.esc(desc)}</em></summary>
      <div class="row-detail-body">${body}</div>
    </details>`;
  }

  function businessDataOverviewHtml() {
    return `
      ...
      <section class="panel system-inner-panel">
      ...
      <details class="panel fold-block system-inner-panel">
  ...
  root.innerHTML = `
    <section class="page-title system-page-title">
      <div>
        <h1>全市场投顾与公募基金产品分析平台</h1>
        <p class="desc">用一页说明数据底座、加工链路、指标层级和系统功能；具体产品与指标从左侧菜单进入。</p>
      </div>
      <div class="title-pills">
        <span class="pill">数据更新至 ${B.esc(dataUpdatedText)}</span>
        <span class="pill">生成时间 ${B.esc(generatedText)}</span>
      </div>
    </section>

    <section class="system-overview-strip" aria-label="系统数据概况">
      ${statTile("数据更新至", dataUpdatedText, `页面包生成 ${generatedText}`, "time")}
      ${statTile("投顾策略", `${count(sourceTotal)} 条`, `${count(channelCount)} 个渠道；天天 ${count(ttfundStrategyCount)} 条`, "strategy")}
      ${statTile("可展示样本", `${count(evidenceRecords)} 条`, "完整策略及当前扩展展示口径", "sample")}
      ${statTile("基金代码", `${count(relatedFundCount)} 只`, `分红事件 ${count(fundDividendRows)} 条`, "fund")}
      ${statTile("基金净值", `${count(fundNavRows)} 条`, `策略依赖净值缺失 ${count(missingStrategyFundNav)}`, "nav")}
      ${statTile("质量状态", qualityWarnings.length ? `${qualityWarnings.length} 项提示` : "无阻断问题", "具体范围见数据质量页", qualityWarnings.length ? "warning" : "ok")}
    </section>

    ${visualSection("一、数据底座", "先说明策略、基金分别拿到了什么，以及统一加工后可以比较什么。", `
      <div class="system-data-map">
        ${dataLane("策略数据", `${count(sourceTotal)} 条统一策略`, ["策略基本信息", "官方业绩曲线", "历史调仓", "当前持仓与基金权重", "业绩基准与费率"], ["统一策略ID", "统一净值回放", "区间收益/回撤/波动", "基准风险资产权重", "比较轨道与同类池"], "strategy")}
        ${dataLane("基金数据", `${count(relatedFundCount)} 只基金代码`, ["基金基本信息", "业绩比较基准", "成立以来复权净值", "分红事件", "类型与管理机构"], ["主份额归并", "区间收益/回撤/波动", "基准资产向量", "两套权益分档", "非权益比较轨道"], "fund")}
      </div>
      <div class="system-coverage-grid" aria-label="投顾策略数据覆盖率">
        ${coverageBar("当前可展示", evidenceRecords, sourceTotal, "进入列表、图表或扩展分析口径")}
        ${coverageBar("官方业绩", officialPerformanceCount, sourceTotal, "可回到渠道曲线核验收益")}
        ${coverageBar("历史调仓", historyRebalanceCount, sourceTotal, "可分析持仓变化和调仓行为")}
        ${coverageBar("统一回放", replayStrategyCount, sourceTotal, "可用统一净值口径复核")}
      </div>
    `)}

    ${visualSection("二、系统主链路", "从采集到正式页面只保留一条主链，质量检查贯穿全程。", `
      <div class="system-architecture">
        <div class="system-architecture-track">
          ${architectureNode("01", "数据来源", "天天/广发 App、公募基金公开源", "source")}
          <i aria-hidden="true">→</i>
          ${architectureNode("02", "增量采集", "ADB、接口与分批同步脚本", "collect")}
          <i aria-hidden="true">→</i>
          ${architectureNode("03", "数据底座", "原始留痕、标准化表、SQLite", "store")}
          <i aria-hidden="true">→</i>
          ${architectureNode("04", "计算治理", "复权净值、基准解析、分类与可比池", "process")}
          <i aria-hidden="true">→</i>
          ${architectureNode("05", "正式输出", "静态页面、数据包、XLS、部署清单", "deliver")}
          <i aria-hidden="true">→</i>
          ${architectureNode("06", "业务使用", "排名、详情、洞察、AI辅助", "use")}
        </div>
        <div class="system-audit-rail"><strong>统一数据稽核</strong><span>重复、字段、日期、覆盖、口径和页面包完整性均进入 hook；warning 显式保留，不静默兜底。</span><a class="link" href="./data-quality.html">查看质量</a></div>
      </div>
    `)}

    ${visualSection("三、数据层级", "区分事实、标准化结果、分析指标和业务结论，避免把中间结果直接当结论。", `
      <div class="system-hierarchy">
        ${hierarchyStep("L1", "事实层", "渠道原始策略、官方曲线、调仓、持仓、基金基准、复权净值", "fact")}
        <i aria-hidden="true">→</i>
        ${hierarchyStep("L2", "标准化层", "统一ID、主份额、日期对齐、基准资产向量、数据状态", "standard")}
        <i aria-hidden="true">→</i>
        ${hierarchyStep("L3", "指标层", "区间收益、最大回撤、波动、权益分档、比较轨道、同类排名", "metric")}
        <i aria-hidden="true">→</i>
        ${hierarchyStep("L4", "业务层", "产品定位、竞品差距、广发优势、营销机会、运营动作", "business")}
      </div>
      <div class="system-boundary-grid">
        ${discussionItem("规模口径", `源表 ${count(sourceTotal)} 条不等于可比样本 ${count(evidenceRecords)} 条；页面筛选、系列归并和数据完整性会改变分母。`)}
        ${discussionItem("分类口径", "基准风险资产权重统一按业绩基准中的权益、商品和另类风险资产合计权重划分，是首层策略分类；不等同风险等级，正式比较还要叠加非权益比较轨道。")}
        ${discussionItem("缺失口径", "缺净值、缺基准或风险指标不足时展示 -- 或质量提示，不强行进入正式经营结论。")}
      </div>
    `)}

    ${visualSection("四、功能划分", "首页只给出功能骨架；产品、指标和稽核明细从对应页面展开。", `
      <div class="system-function-grid">
        ${functionCard("全市场产品排名", "投顾与基金混排、收益风险点阵、分档和轨道筛选", "./mixed-performance-scatter.html")}
        ${functionCard("策略列表与详情", "基础信息、业绩曲线、持仓基金、调仓和基准证据", "./strategies.html")}
        ${functionCard("数据洞察", "市场结构、广发竞争位置、持仓机会和调仓复盘", "./insights.html")}
        ${functionCard("策略比较", "选取多只策略横向比较业绩、配置和选基效果", "./compare.html")}
        ${functionCard("AI选策略", "用自然语言调用本机模型筛选和解释产品", "./ai-strategy.html")}
        ${functionCard("数据质量", "查看缺失、持续 warning、规则和本轮更新影响", "./data-quality.html")}
      </div>
    `)}

    ${systemModule("后续需要确定的三个口径", "经营口径、评价权重、数据 SLA", `
      <div class="system-discussion-list">
        ${discussionItem("经营口径", "明确哪些排名、可比池和优势产品进入周/月度经营材料，哪些只作为研究观察。")}
        ${discussionItem("评价权重", "确定收益、风险控制、稳定性、客户适配、赛道稀缺性和营销可解释性的权重。")}
        ${discussionItem("数据 SLA", "明确日更、周更、月更范围，以及 warning 出现时是否暂停对外结论。")}
      </div>
    `)}

    ${systemModule("详细数据与字段审计", "渠道、字段覆盖、指标口径和明细表按需展开", businessDataOverviewHtml())}
  `;
  setupMetricAnalyzer();
})();
